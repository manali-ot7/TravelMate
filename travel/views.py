...
def home(request):
    ...
    hotels = Hotel.objects.select_related("destination").filter(property_type=Hotel.HOTEL)
...
def service_search(request, service):
    ...
    if active_tab["kind"] == "transport":
        ...
        queryset = Hotel.objects.select_related("destination").filter(property_type=Hotel.HOTEL)
        if search_place:
            queryset = queryset.filter(
...
def hotels(request):
    hotel_list = Hotel.objects.select_related("destination").filter(property_type=Hotel.HOTEL)
